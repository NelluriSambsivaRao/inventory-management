...
con = sqlt.connect(host = "localhost", user = "root", passwd = "Siva@026920", database = "inventory")
cursor = con.cursor()
def show_item():
...
def show_sale():
    ...
    df=pd.read_sql("select * from smaster where sdate between '{}' and '{}';".format(bdate,edate),con)
    print(tabulate(df,headers= 'keys', tablefmt='psql',showindex = False))
def show_purchase():
    bdate=input("enter beginning date")
    edate=input("enter end date")
    df=pd.read_sql("select * from pmaster where pdate between '{}' and '{}';".format(bdate,edate),con)
    print(tabulate(df,headers= 'keys', tablefmt='psql',showindex = False))
def best_product():
